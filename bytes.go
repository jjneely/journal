...
func NewByteValueType(width int64, null []byte) *ByteValueType {
	b := new(ByteValueType)
	b.width = width
	for i := int64(len(null)); i < width; i++ {
...
func (t *ByteValueType) Width() int64 {
	return t.width
}
...
func (t *ByteValueType) Decode(buffer []byte) Values {
	b := make([][]byte, 0)
	for i := int64(0); i < int64(len(buffer)); i += t.width {
